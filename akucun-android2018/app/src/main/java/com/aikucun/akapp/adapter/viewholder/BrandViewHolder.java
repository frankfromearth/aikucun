...
public class BrandViewHolder extends BaseViewHolder<Product> {
    ...
    public TextView consultationBtn;
    ...
    //虚拟商品说明
    public TextView not_refundable_text;
    //结算价
    public TextView settlement_price_text;
    //代购费
    public TextView purchase_fee_text;
    //代购费x2
    public TextView double_purchase_fee_text;
    //吊牌价
    public TextView tag_price_text;
    //评论
    public EasyRecyclerView commentRecyclerView;
    //促销
    public ImageView promotion_iv;
    ...
    public BrandViewHolder(ViewGroup parent) {
        super(parent, R.layout.adapter_brand_item);
        ...
        consultationBtn = $(R.id.consultation);
        ...
        price_layout = $(R.id.price_layout);
        settlement_price_text = $(R.id.settlement_price_text);
        purchase_fee_text = $(R.id.purchase_fee_text);
        double_purchase_fee_text = $(R.id.double_purchase_fee_text);
        tag_price_text = $(R.id.tag_price_text);
        not_refundable_text = $(R.id.not_refundable_text);
        promotion_iv = $(R.id.promotion_iv);
...
*/
    ...
    public void setData(Product data) {
        ...
        if (0 == data.getProductType()) {
            ...
            int bohuojia = data.getXiaoshoujia();// FastForwardPopWindow.getDescAmount(data.getDesc());
            if (AppContext.getInstance().getUserInfo().getViplevel() > 0 && bohuojia > 0) {
                price_layout.setVisibility(View.VISIBLE);
                settlement_price_text.setText(StringUtils.getNum2Decimal((float) data.getJiesuanjia() / 100 + ""));

                if (bohuojia > 0) {
                    float temp1 = (bohuojia - data.getJiesuanjia()) / 100;
                    if (data.getSalestype() == 1) {
                        String temp = StringUtils.getDecimal(temp1 / 2);
                        purchase_fee_text.setText(temp);
                        double_purchase_fee_text.setVisibility(View.VISIBLE);
                        promotion_iv.setVisibility(View.VISIBLE);
                    } else {
                        String temp = StringUtils.getDecimal(temp1);
                        purchase_fee_text.setText(temp);
                        double_purchase_fee_text.setVisibility(View.GONE);
                        promotion_iv.setVisibility(View.GONE);
                    }
                    tag_price_text.setText("¥" + StringUtils.getNum2Decimal((float) data.getDiaopaijia() / 100 + ""));
                    tag_price_text.getPaint().setFlags(Paint.STRIKE_THRU_TEXT_FLAG);
                }
            } else {
...
//                String endStr = AppContext.getInstance().getResources().getString(R.string.not_refundable);
...
//                tvContent.setText(data.getDesc());
            if (data.getIsvirtual() == 1) {
                not_refundable_text.setVisibility(View.VISIBLE);
            } else {
                not_refundable_text.setVisibility(View.GONE);
            }
            tvContent.setText(data.getDesc());
