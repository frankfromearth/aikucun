package cn.sucang.widget.spinkit.animation;
...
import cn.sucang.widget.spinkit.animation.interpolator.KeyFrameInterpolator;
...
public class SpriteAnimatorBuilder
{
    ...
    public SpriteAnimatorBuilder translateXPercentage(float fractions[], Float...
    ...
    public SpriteAnimatorBuilder translateYPercentage(float[] fractions, Float...
    ...
    private void ensurePair(int fractionsLength, int valuesLength)
    {
        ...
        {
            throw new IllegalStateException(String.format(Locale.getDefault(), "The fractions" +
                    ".length must equal values.length, " + "fraction.length[%d], values" +
                    ".length[%d]", fractionsLength, valuesLength));
        }
    }
